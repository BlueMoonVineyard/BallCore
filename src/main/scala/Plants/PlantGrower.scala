	...
	def grow(block: Block, what: PlantType): Boolean =
		what match
			case PlantType.ageable(mat, _) =>
				block.setBlockData(
					block.getBlockData().asInstanceOf[Ageable].tap(x => x.setAge((x.getAge() + 1).min(x.getMaximumAge()))), true
				)
				block.getBlockData().asInstanceOf[Ageable].pipe(x => x.getMaximumAge() == x.getAge())
			case PlantType.generateTree(mat, kind, _) =>
				val consumer: Consumer[BlockState] = state => {}
				block.setType(Material.AIR, true)
				if !block.getWorld().generateTree(block.getLocation(), java.util.Random(), kind, consumer) then
					block.setType(Material.DEAD_BUSH, true)
				true
