...
import org.locationtech.jts.geom.{Coordinate, GeometryCollection, GeometryFactory, Polygon}
      ...
      .flatMap { hearts =>
        ...
        val heartsOutsidePolygon =
          hearts.filterNot((x, _, z) =>
            polygon.contains(
              geometryFactory.createPoint(Coordinate(x.toDouble, z.toDouble))
            )
          )
        ...
        if actualArea > expectedArea then
          IO.pure(
            Left(
              PolygonAdjustmentError.polygonTooLarge(expectedArea, actualArea)
            )
          )
          ...
          sql
            ...
            .map { (entry, data) =>
              data.beaconRTree = RTree.update(
                data.beaconRTree,
                data.beaconIDsToRTreeEntries.get(beacon).toSeq.flatten,
                entry
              )
