    ...
    def adjustBlock(block: Block): Block =
        block.getType() match
            ...
            case WEEPING_VINES =>
                ...
                while above.getType() == block.getType() || above.getType() == Material.WEEPING_VINES_PLANT do
                    above = above.getRelative(BlockFace.UP)
                above
            // cave vines also come from the ceiling
            case CAVE_VINES =>
                var above = block.getRelative(BlockFace.UP)
                while above.getType() == block.getType() || above.getType() == Material.CAVE_VINES_PLANT do
                    above = above.getRelative(BlockFace.UP)
                above
            ...
            case TWISTING_VINES =>
                var below = block.getRelative(BlockFace.DOWN)
                while below.getType() == block.getType() || below.getType() == Material.TWISTING_VINES_PLANT do
                    below = below.getRelative(BlockFace.DOWN)
                below
            // dripleafs come from the floor or a dripleaf stem
            case BIG_DRIPLEAF =>
                var below = block.getRelative(BlockFace.DOWN)
                while below.getType() == block.getType() || below.getType() == Material.BIG_DRIPLEAF_STEM do
                    below = below.getRelative(BlockFace.DOWN)
                below
            ...
            case POINTED_DRIPSTONE =>
                ...
                var direction = block.getRelative(dripstone.getVerticalDirection().getOppositeFace());
