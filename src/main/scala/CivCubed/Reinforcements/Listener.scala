...
import CivCubed.Groups.{GroupError, GroupManager, Permissions, nullUUID}
...
import CivCubed.Groups.Position
import CivCubed.WebHooks.WebHookManager
import cats.data.OptionT
import CivCubed.PrimeTime.{PrimeTimeManager, PrimeTimeResult}
...
import CivCubed.Groups.SubgroupID
...
class Listener(using
    ...
    private def checkAt(
        ...
        if player.hasPermission("civcubed.bypass") then Right(true)
        else
