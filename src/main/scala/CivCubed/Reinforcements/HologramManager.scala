...
import CivCubed.DataStructures.Delay
import CivCubed.Folia.{FireAndForget, LocationExecutionContext}
import net.kyori.adventure.text.Component
import org.bukkit.Location
import org.bukkit.block.Block
import org.bukkit.entity.Display.Billboard
import org.bukkit.entity.{Player, TextDisplay}
...
class Hologram(val createAt: Location)(using JavaPlugin):
    ...
    private val lineHeight = 0.23
    private val spaceBetweenLines = 0.02
    var lines: List[TextDisplay] = List[TextDisplay]()
    private var position: Location = createAt
    ...
    def relocate(to: Location): Unit =
        ...
        FireAndForget {
            ...
            lines.zipWithIndex.foreach { (elem, idx) =>
                currentY -= lineHeight
                if idx > 0 then currentY -= spaceBetweenLines
...
class HologramManager(using p: JavaPlugin):
    private val holos =
        concurrent.TrieMap[(Int, Int, Int, UUID), (Hologram, () => Unit)]()

    private def getHologram(at: Location): Hologram =
        given ctx: ExecutionContext = LocationExecutionContext(at)

        val key =
            (at.getBlockX, at.getBlockY, at.getBlockZ, at.getWorld.getUID)
        holos.get(key) match
            case None =>
