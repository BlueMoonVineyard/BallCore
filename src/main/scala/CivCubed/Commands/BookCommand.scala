...
import CivCubed.SpawnInventory.OresAndYou
import CivCubed.SpawnInventory.FingerprintsAndYou
import CivCubed.SpawnInventory.WorkstationsAndYou
import CivCubed.SpawnInventory.BattlesAndYou
import CivCubed.SpawnInventory.SigilsAndYou
import CivCubed.SpawnInventory.SmelteryAndYou
import CivCubed.Advancements.ViewOres
import CivCubed.SpawnInventory.HeartsAndYou

class BookCommand(using
    storage: CivCubed.Acclimation.Storage,
    sql: SQLManager,
    p: Plugin,
):
    val node =
        CommandTree("book")
            ...
            .`then`(
                LiteralArgument("spawnbook")
                    .executesPlayer({ (sender, args) =>
                        sender.openBook(CivCubed.SpawnInventory.Book.book)
                    }: PlayerCommandExecutor)
            )
