...
import CivCubed.Folia.{EntityExecutionContext, FireAndForget}
...
class DamageActor()(using p: Plugin) extends Actor[DamageMessage]:
    private var damages: Map[UUID, Map[UUID, Double]] =
        ...
        m match
            case DamageMessage.damage(from, against, damage) =>
                ...
                damages = damages.updated(
                    against,
                    inner.updatedWith(from) { x =>
                        Some(x.getOrElse(0.0) + damage)
                    },
                )
...
class DamageListener(using da: DamageActor) extends Listener:
    private def getPlayerOrigin(e: Entity): Player =
        e match
                    ...
                    val searched =
                        attacker.getInventory
                            ...
                            .find((_, is) => {
                                ...
                                if item.isEmpty || !item.get.isInstanceOf[Sigil]
                                then false
                                else
                    ...
                    searched match
                        ...
                        case Some(value) =>
                            ...
                            if stack.getAmount == 1 then
                                stack.setItemMeta(
                                    sigil.itemMetaForBound(stack, killed)
                                )
                            else
                                stack.setAmount(stack.getAmount - 1)
                                val newStack = stack.clone()
                                newStack.setAmount(1)
                                newStack.setItemMeta(
                                    sigil.itemMetaForBound(newStack, killed)
                                )
                                if !attacker.getInventory
                                        .addItem(newStack)
                                        .isEmpty
                                then
                                    val _ = attacker.getWorld
                                        .dropItemNaturally(
