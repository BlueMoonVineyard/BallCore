...
package CivCubed.MapCloning

import CivCubed.MapCloning.MapCloningListener.persistenceKeyMapCopyStatus
...
object MapCloningListener:
    val persistenceKeyMapCopyStatus: NamespacedKey =
        NamespacedKey("civcubed", "map_copy_status")
...
class MapCloningListener extends Listener:
    ...
    def prepareCraftEvent(pice: PrepareItemCraftEvent): Unit =
        ...
        if !pice.getInventory.getContents.exists(x =>
                x != null && x.getType == Material.MAP
            )
        then return
    ...
    def prepareResultEvent(per: PrepareResultEvent): Unit =
        ...
        if !per.getInventory.getContents.exists(x =>
                x != null && x.getType == Material.MAP
            )
        then return
