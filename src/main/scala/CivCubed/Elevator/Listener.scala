...
class Listener extends org.bukkit.event.Listener:
    ...
    private def attemptTeleport(
        potentialElevator: Block,
        player: Player,
    ): Boolean =
        if potentialElevator.getType() != elevatorMaterial then return false

        val up1 = potentialElevator.getRelative(BlockFace.UP)
        val up2 = up1.getRelative(BlockFace.UP)

        if !up1.isPassable() || !up2.isPassable() then return false
        ...
        potentialElevator
            .getWorld()
            .playSound(sound, loc.getX(), loc.getY(), loc.getZ())

        true

    @EventHandler
    def onSneak(event: PlayerToggleSneakEvent): Unit =
        if !event.isSneaking() then return ()
        ...
        if block.getType() != elevatorMaterial then return ()

        if !(block.getY() - 1 to block.getWorld().getMinHeight() by -1)
                .map(fromY)
                .exists { block =>
        ...
        then
            player.sendServerMessage(
                txt"There aren't any lodestones below you."
            )
    ...
    def onJump(event: PlayerJumpEvent): Unit =
        ...
        if !(block.getY() + 1 to block.getWorld().getMaxHeight())
                .map(fromY)
                .exists { block =>
                    attemptTeleport(block, player)
                }
        then
            player.sendServerMessage(
                txt"There aren't any lodestones above you."
            )
