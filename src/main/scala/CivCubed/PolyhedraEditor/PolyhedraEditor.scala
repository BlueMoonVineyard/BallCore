package CivCubed.PolyhedraEditor

import CivCubed.Groups.{GroupID, GroupManager, Position, Subclaims, SubgroupID}
        ...
        state match
            ...
            case editing(editingState) =>
                editingState.state match
                    ...
                    case CivCubed.PolyhedraEditor.VolumeEditingState
                            .lookingAt(corner) =>
                        player.sendActionBar(
...
class PolyhedraEditor(using
    ...
    def done(player: Player): Unit =
        ...
        val volumes = state.done().map { volume =>
            ...
            CivCubed.Groups.Volume(into(volume.cornerA), into(volume.cornerB))
        }
