  ...
  def click(block: Block): State =
    state match
      case lookingAt(volumeIndex) =>
        ...
        copy(
          volumes = newVolumes,
          state = editing(EditingVolume(volume, VolumeEditingState.none()))
        )
    ...
    state match
      ...
      case lookingAt(volumeIndex) =>
        volumes.zipWithIndex.foreach { (volume, idx) =>
          if idx == volumeIndex then volume.render(player, draggingTeal)
...
class PolyhedraEditor(using p: Plugin, sql: SQLManager, gm: GroupManager):
  ...
  def create(
    ...
    sql.useBlocking(gm.getSubclaims(group)) match
      ...
      case Right(claims) =>
        val volumes =
          claims.getOrElse(subgroup, Subclaims(Nil)).volumes.map { volume =>
