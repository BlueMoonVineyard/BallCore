...
object Mining:
  ...
  val drops: List[Drops] = List(
    ...
    ores.map((cardinal, ore) =>
      Drops(0 to 320, 1 to 4, 0.004, ore, WorldLocation.cardinal(cardinal))
    )
  }.concat {
    // blue cardinal ores
    val ores = List(
    ...
    ores.map((cardinal, ore) =>
      Drops(-64 to 16, 1 to 3, 0.0010854, ore, WorldLocation.cardinal(cardinal))
    )
  }.concat {
    // white quadrant ores
    val ores = List(
    ...
    ores.map((quadrant, ore) =>
      Drops(-64 to 320, 1 to 4, 0.014, ore, WorldLocation.quadrant(quadrant))
    )
  }.concat {
    ...
    ores.map((quadrant, ore) =>
      Drops(-64 to 32, 1 to 4, 0.002874, ore, WorldLocation.quadrant(quadrant))
    )
  }.concat {
    // red quadrant ores
    val ores = List(
    ...
    ores.map((quadrant, ore) =>
      Drops(-16 to 112, 1 to 5, 0.02, ore, WorldLocation.quadrant(quadrant))
    )
  }
...
object WorldLocation:
  ...
  val norths: List[WorldLocation] =
    List(quadrant(northwest), quadrant(northeast), cardinal(north), everywhere)
  val wests: List[WorldLocation] =
    List(quadrant(northwest), quadrant(southwest), cardinal(west), everywhere)
  val souths: List[WorldLocation] =
    List(quadrant(southwest), quadrant(southeast), cardinal(south), everywhere)
  val easts: List[WorldLocation] =
    List(quadrant(northeast), quadrant(southeast), cardinal(east), everywhere)
  ...
  def onBreakBlock(event: BlockBreakEvent): Unit =
    ...
    if !it3 then
      return if !Mining.stoneBlocks.contains(event.getBlock.getType) then
        return if Mining.oceans.contains(event.getBlock.getBiome) then return
