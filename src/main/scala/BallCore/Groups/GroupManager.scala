...
class GroupManager()(using sql: Storage.SQLManager):
  ...
  def deleteGroup(as: UserID, group: GroupID)(using
    ...
    EitherT
      ...
      .flatMap { _ =>
        EitherT.right(
          sql.commandIO(sql"DELETE FROM GroupStates WHERE ID = $uuid;", group)
        )
      }
      .map(_ => ())
  ...
  private def getAll(
    ...
    for {
      ...
      users = gm.map { membership =>
        (
          membership.userID,
          grm.filter(_.userID == membership.userID).map(_.roleID).toSet
        )
      }.toMap,
      subgroups = subgroups
    )
  ...
  def setRolePermissions(
    ...
    getAll(groupID)
      ...
      .flatMap { _ =>
        EitherT.right(
          sudoSetRolePermissions(groupID, roleID, permissions).map(_ => ())
        )
      }
  ...
  def setSubgroupRolePermissions(
    ...
    getAll(groupID)
      ...
      .guard(GroupError.MustHavePermission) { gs =>
        permissions.forall { (perm, mode) =>
          if mode == RuleMode.Allow then gs.check(perm, as, subgroupID)
          else true
        }
      }
      .flatMap { _ =>
  ...
  def getSubclaims(
                    groupID: GroupID
                  )(using s: Session[IO]): IO[Either[GroupError, Map[SubgroupID, Subclaims]]] =
    EitherT
      .right(
        sql.queryListIO(
          sql"""
