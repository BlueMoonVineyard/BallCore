...
import org.bukkit.inventory.{BlastingRecipe, FurnaceRecipe, ShapedRecipe, ShapelessRecipe}
...
case class OreVariants(
  ...
  def register(group: ItemGroup, registry: ItemRegistry, serv: Server): Unit =
    ...
    def ingotFromBlockKey(in: CustomItemStack) =
      NamespacedKey(
        group.key.getNamespace,
        group.key.getKey + "_ingot_from_block_" + in.id.getKey.toLowerCase()
      )
    ...
    def ingotFromNuggetKey(in: CustomItemStack) =
      NamespacedKey(
        group.key.getNamespace,
        group.key.getKey + "_ingot_from_nugget_" + in.id.getKey.toLowerCase()
      )
    ...
    val nuggetRecipe =
      ShapelessRecipe(nuggetKey(nugget), nugget.clone().tap(_.setAmount(9)))
...
object Helpers:
  def factory(
    ...
    OreVariants(
      withCustomModelData(
        CustomItemStack
          .make(NamespacedKey("ballcore", s"raw_$id"), raw, txt"Raw $name"),
        num + 0
      ),
      withCustomModelData(
