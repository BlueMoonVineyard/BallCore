...
import BallCore.Folia.{ChunkExecutionContext, FireAndForget, LocationExecutionContext}
...
class PlantBatchManager()(using sql: SQLManager, p: Plugin, c: Clock)
  ...
  private def millisToNextHour(): Long =
    val nextHour =
      LocalDateTime.now().plus(1, DateUnit.hour).truncatedTo(DateUnit.hour)
  ...
  private val plants =
    mutable.Map[(Int, Int, UUID), Map[(Int, Int, Int), Dirty[DBPlantData]]]()
    ...
    m match
      case PlantMsg.startGrowing(what, where) =>
        ...
        set(
          ...
          oz,
          y,
          Dirty(DBPlantData(cx, cz, world, ox, oz, y, what, 0, 0), true, false)
        )
      ...
      case PlantMsg.plantsGrewPartially(where) =>
        where.foreach { blk =>
          ...
          update(cx, cz, world, ox, oz, y) { d =>
            d.copy(inner = d.inner.copy(incompleteGrowthAdvancements = 0))
          }
        }
      case PlantMsg.inspect(where, player) =>
        ...
        FireAndForget {
          get(cx, cz, world).get(ox, oz, y).foreach { plant =>
            ...
            val actualSeason =
              Datekeeping.time().month.toInt.pipe(Month.fromOrdinal).season
            ...
            if plant.deleted then plantKey -> plant
              ...
              val rightSeason =
                plant.inner.what.growingSeason growsWithin Datekeeping
                  ...
                  .season
              val rightClimate =
                plant.inner.what.growingClimate growsWithin Await.result(
                  Future {
              ...
              val timePassed =
                (plant.inner.ageIngameHours + 1) % plant.inner.what.plant
                  .hours() == 0
              val incrBy =
                if rightSeason && rightClimate && timePassed then 1
                else 0
              plantKey -> plant.copy(
