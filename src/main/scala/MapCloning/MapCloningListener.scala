...
class MapCloningListener extends Listener:
  ...
  def prepareCraftEvent(pice: PrepareItemCraftEvent): Unit =
    if pice.getInventory().getResult() == null || pice
      .getInventory()
      .getResult()
    ...
    val inputMap = pice
      ...
      .get
    val inputMapGeneration = inputMap
      .getItemMeta()
    ...
    inputMapGeneration match
      case MapCopyStatus.original | MapCopyStatus.copyOfOriginal =>
        if inputMapGeneration == MapCopyStatus.original then
          ...
          resultMapMeta.lore(
            ju.List.of(Component.translatable("book.generation.1").pipe(greyIt))
          )
        else
          resultMapMeta
            .getPersistentDataContainer()
          ...
          resultMapMeta.lore(
            ju.List.of(Component.translatable("book.generation.2").pipe(greyIt))
          )
