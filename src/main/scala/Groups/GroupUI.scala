...
class GroupManagementProgram(using
  ...
  def canBindToHeart(self: UserID): Boolean =
    sql
      .useBlocking(cbm.getBeaconFor(self))
        ...
        .foreach { x =>
          Item(Material.PLAYER_HEAD, displayName = Some(txt"${x.getName()}")) {
            ...
            if roles.length > 0 then
              ...
              Lore(
                txt"Roles"
                  .style(NamedTextColor.WHITE, TextDecoration.UNDERLINED)
              )
              roles.foreach { x =>
        ...
        Button(
          ...
          if x.permissions.size > 0 then
            ...
            x.permissions.toList.sortBy(_._1.ordinal).foreach { x =>
              ...
              else Lore(txt"- ✖ ${p.displayName()}".style(NamedTextColor.RED))
            }
        }
      }
    }
    ...
    msg match
      ...
      case Message.CreateSubgroup =>
        services
          ...
          .map { name =>
            sql.useBlocking(
              gm.createSubgroup(model.userID, model.group.metadata.id, name)
                .value
            ) match
              case Left(err) =>
                services.notify(
            ...
            val group = sql
              .useBlocking(gm.getGroup(model.group.metadata.id).value)
              .toOption
              .get
  ...
  override def update(msg: Message, model: Model)(using
    ...
    msg match
      case Message.DeleteRole =>
        sql.useBlocking(
          gm.deleteRole(model.userID, model.role.id, model.groupID).value
        ) match
          case Left(err) =>
            services.notify(
                  ...
                  Some(
                    subgroup.copy(permissions =
                      subgroup.permissions
                        .updated(model.role.id, newPermissions)
                    )
                  )
                )
      case Message.GoBack =>
        ...
        Permissions.values.foreach { x =>
          ...
          Button(x.displayItem(), txt"${name}", Message.TogglePermission(x)) {
            ...
            model.subgroup match
              case None =>
                permissions.get(x) match
                  case None =>
                    Lore(
                      txt"This role does not affect this permission".color(
                        NamedTextColor.GRAY
                      )
                    )
            ...
            permissions.get(x) match
              ...
              case Some(RuleMode.Allow) =>
                Lore(
                  txt"Click to toggle ignore/${txt"allow".color(NamedTextColor.WHITE)}/deny"
                    .color(NamedTextColor.GRAY)
                )
              case Some(RuleMode.Deny) =>
                Lore(
                  txt"Click to toggle ignore/allow/${txt"deny".color(NamedTextColor.WHITE)}"
                    .color(NamedTextColor.GRAY)
                )
          }
        }
  ...
  override def update(msg: Message, model: Model)(using
    ...
    msg match
      case Message.ClickInvite(user, group) =>
        model.copy(mode =
          Mode.ViewingInvite(user, model.invites.find(_._1 == user).get._2)
        )
      ...
      OutlinePane(0, 0, 1, rows) {
        model.invites.foreach { invite =>
          ...
          Button(
            ...
            Skull(player)
            Lore(
              txt"Invited you to ${txt"${invite._2.metadata.name}".color(NamedTextColor.GREEN)}"
    ...
    Root(txt"Accept / Reject Invite?", 1) {
      ...
      OutlinePane(4, 0, 1, 1) {
        ...
        Item(
          Material.PLAYER_HEAD,
          displayName =
            Some(txt"§${player.getName()}".style(NamedTextColor.WHITE))
        ) {
          Skull(player)
          Lore(
    ...
    msg match
      ...
      case Message.CreateGroup =>
        ...
        answer.map { result =>
          ...
          model.copy(groups =
            sql.useBlocking(gm.userGroups(model.userID).value).toOption.get
          )
        }
