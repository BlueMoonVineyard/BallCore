...
import net.kyori.adventure.text.format.{NamedTextColor, TextColor, TextDecoration}
  ...
  def handle(m: ChatMessage): Unit =
    m match
      ...
      case ChatMessage.chattingInGroup(p, group) =>
        ...
        p.sendMessage(
          txt"You are now chatting in a group".color(NamedTextColor.GREEN)
        )
      case ChatMessage.chattingInGlobal(p) =>
        states += p -> PlayerState.globalChat
        p.sendMessage(
          txt"You are now chatting in global chat".color(NamedTextColor.GREEN)
        )
      case ChatMessage.chattingInLocal(p) =>
        states += p -> PlayerState.localChat
        p.sendMessage(
          txt"You are now chatting in local chat".color(NamedTextColor.GREEN)
        )
