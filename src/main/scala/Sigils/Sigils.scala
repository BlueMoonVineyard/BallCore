...
import BallCore.CustomItems.{CustomItem, CustomItemStack, ItemGroup, ItemRegistry}
...
object Sigil:
  ...
  itemStack.setItemMeta(
    itemStack.getItemMeta().tap(_.setCustomModelData(CustomModelData.Empty.num))
  )
...
class Sigil extends CustomItem:
  ...
  def itemMetaForBound(from: ItemStack, toPlayer: Player): ItemMeta =
    ...
    im.displayName(
      txt"Bound Sigil".color(NamedTextColor.YELLOW).not(TextDecoration.ITALIC)
    )
    im.lore(
      List(
