...
class ShopChest(using ItemRegistry) extends CustomItem, Listeners.BlockLeftClicked:
    ...
    private def addify(inv: Inventory)(toAdd: (ItemStack, Int)): Unit =
        ...
        val stacks = item.clone().tap(_.setAmount(amount % item.getMaxStackSize())) :: List.fill(amount / item.getMaxStackSize())(item.clone().tap(_.setAmount(item.getMaxStackSize())))
    ...
    override def onBlockLeftClicked(event: PlayerInteractEvent): Unit =
        ...
        if event.getItem() == null || event.getItem().getType() == Material.AIR then
            ...
            orders.lift(state.index) match
                ...
                case Some(order) =>
                    val possibleExchanges = order.countPossibleExchanges(inv.getStorageContents().iterator)
                    ...
                    player.sendServerMessage(txt"Item: ${order.selling._1.displayName().hoverEvent(order.selling._1)} × ${order.selling._2} (${possibleExchanges} sales available)")
            ...
            val orders = SellOrderDescription.enumerateFrom(inv.getStorageContents().iterator).toList
            val state = states.getOrElseUpdate(event.getPlayer().getUniqueId(), PurchasingState(-1))
            val player = event.getPlayer()

            orders.lift(state.index) match
                case Some(order) if order.price._1.matches(event.getItem()) =>
                    val playerInventory = event.getPlayer().getInventory()
                    order.perform(playerInventory.getStorageContents().iterator, playerInventory.getStorageContents().iterator, addify(playerInventory), inv.getStorageContents().iterator, inv.getStorageContents().iterator, addify(inv)) match
                        case Left(err) =>
                            err match
                                case ExchangeError.buyerCantAfford(has, needed) =>
                                    player.sendServerMessage(txt"You can't afford that purchase (it requires ${needed} ${order.price._1.displayName()} but you only have ${has})")
                                case ExchangeError.buyerCantReceive =>
                                    player.sendServerMessage(txt"You can't store the purchased goods in your inventory")
                                case ExchangeError.sellerCantAfford(has, needed) =>
                                    player.sendServerMessage(txt"The chest doesn't have enough items to sell (it requires ${needed} ${order.selling._1.displayName()} but it only has ${has})")
                                case ExchangeError.sellerCantReceive =>
                                    player.sendServerMessage(txt"The chest doesn't have enough space to store the payment")
                                case ExchangeError.unknownPrice =>
                                    player.sendServerMessage(txt"The price is invalid")
                            
                        case Right(_) =>
                            player.sendServerMessage(txt"You paid ${order.price._1.displayName()} × ${order.price._2} and received ${order.selling._1.displayName().hoverEvent(order.selling._1)} × ${order.selling._2} ")
                case Some(order) =>
                    player.sendServerMessage(txt"Invalid payment, expected ${order.price._2}")
                case _ =>
                    ()
