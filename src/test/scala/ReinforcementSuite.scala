...
import BallCore.Reinforcements.{ReinforcementError, ReinforcementState, ReinforcementTypes}
...
class ReinforcementSuite extends munit.FunSuite {
  ...
  sql.test("block break and replace") { implicit sql =>
    ...
    val res1 =
      rm.reinforce(u1, gid, nullUUID, 0, 0, 0, world, ReinforcementTypes.Stone)
    assert(res1 == Right(()), res1)
