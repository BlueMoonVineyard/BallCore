...
class OTTSuite extends munit.FunSuite {
    ...
    sql.test("basic functionality") { implicit sql =>
        ...
        val res1 = sql.useBlocking(
            sql.withS(ott.requestTeleportTo(joinedPlayer, establishedPlayer))
        )
        assertEquals(res1, Right(()), "the initial request should be fine")

        val res2 = sql.useBlocking(
            sql.withS(ott.acceptTeleportOf(establishedPlayer, joinedPlayer))
        )
        assertEquals(res2, Right(()), "the accepting should be fine")

        val res2_1 = sql.useBlocking(
            sql.withS(ott.acceptTeleportOf(establishedPlayer, joinedPlayer))
        )
        assertEquals(
            res2_1,
            Left(OTTError.isNotTeleportingToYou),
            "shouldn't do multiple times",
        )

        val someOtherPlayer = mockServerSingleton.addPlayer()

        val res3 = sql.useBlocking(
            sql.withS(ott.requestTeleportTo(joinedPlayer, someOtherPlayer))
        )
        assertEquals(
            res3,
            Left(OTTError.alreadyUsedTeleport),
            "you used your one teleport",
        )

        val res4 = sql.useBlocking(
            sql.withS(ott.acceptTeleportOf(joinedPlayer, someOtherPlayer))
        )
        assertEquals(
            res4,
            Left(OTTError.isNotTeleportingToYou),
            "they weren't going to teleport to you",
        )

        hooks.it = false

        val res5 = sql.useBlocking(
            sql.withS(ott.requestTeleportTo(someOtherPlayer, joinedPlayer))
        )
