...
trait KeyVal:
    def set[A](player: UUID, key: String, value: A)(using Encoder[A], Decoder[A], Session[IO]): IO[Unit]
    def get[A](player: UUID, key: String)(using Encoder[A], Decoder[A], Session[IO]): IO[Option[A]]
    def remove(player: UUID, key: String)(using Session[IO]): IO[Unit]

    def set[A](superkey: String, key: String, value: A)(using Encoder[A], Decoder[A], Session[IO]): IO[Unit]
    def get[A](superkey: String, key: String)(using Encoder[A], Decoder[A], Session[IO]): IO[Option[A]]
