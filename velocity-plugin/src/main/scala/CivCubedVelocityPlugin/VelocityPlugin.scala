package CivCubedVelocityPlugin

import CivCubed.LinkedAccounts.{LinkedAccountError, LinkedAccountsManager}
import CivCubed.Storage.{Config, SQLManager}
...
class HTTP(apiKey: String)(using lam: LinkedAccountsManager):
    ...
    private val linkStartRoute: Route =
    ...
    private val linkCompleteRoute: Route =
    ...
    def register()(using l: Logger): () => Future[Unit] =
        val confText = new String(
            classOf[HTTP]
                .getResourceAsStream("/civcubed-akka.conf")
                .readAllBytes(),
        ...
        given system: ActorSystem[Unit] =
            ActorSystem(Behaviors.empty, "civcubed", setup)
...
object VerifyCommand:
    private def explain(s: LinkedAccountError): String =
        s match
    ...
    def make()(using lam: LinkedAccountsManager): BrigadierCommand =
        val node = LiteralArgumentBuilder
            ...
            .executes { context =>
                ...
                val linkCode =
                    lam.startLinkProcessFromMinecraft(player.getUniqueId)
                linkCode match
                    case Left(err) =>
                        ...
                        val link = txt("/link-code use")
                            ...
                            .clickEvent(
                                ClickEvent.copyToClipboard(
                                    s"/link-code use code:$linkCode"
                                )
                            )
                            .hoverEvent(
                                HoverEvent.showText(
                                    txt"/link-code use code:$linkCode"
                                )
                            )
                        player.sendServerMessage(
                            txt"Your link code is ${txt(linkCode.toUpperCase())
                                    .color(Colors.yellow)
                                    .decorate(TextDecoration.BOLD)}. Use $link (click to copy the command) on Discord to link your Minecraft account to your Discord account."
                        )
            ...
            .`then`(
                RequiredArgumentBuilder
                    ...
                    .executes { context =>
                        ...
                        val result =
                            lam.finishLinkProcessFromMinecraft(
                                linkCode,
                                player.getUniqueId,
                            )
                        result match
